...
namespace FlowerEShopAPI.BL.Services
{
    public class SearchService : ISearchService
    {
        ...
        public async Task<List<Shop>> SearchShops(string searchQuery, decimal? priceStart, decimal? priceEnd, string status = "All")
        {
            ...
            if (shopList.Where(shop => regex.IsMatch(shop.Name) || regex.IsMatch(shop.Location) || shop.Products.Where(product => regex.IsMatch(product.Title) || regex.IsMatch(product.Category)).Any()).Any() || trimmedSearchQuery == "")
            {
